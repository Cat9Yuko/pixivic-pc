    ...
    <div class="page-padding">
      <main class="detail-content">
        <figure class="detail-content__figure">
          <el-image
            v-if="illustDetail.xrestrict==0&&illustDetail.sanityLevel<=(user ? 3 : 2)"
  ...
  methods: {
    ...
    handleData(data) {
      ...
      return {
        ...
        setu: data.setu || !!((data.xrestrict === 1 || data.sanityLevel > (this.user ? 3 : 2))) && this.user.username !== 'pixivic',
      ...
      this.$api.detail
        ...
        .then(res => {
          if (res.data.data) {
            ...
            this.pictureList = this.pictureList.concat(data).filter(item => item.xrestrict === 0 && item.sanityLevel <= (this.user ? 3 : 2));
          }
        })
        .catch(err => {
          console.error(err);
        ...
        .then(res => {
          if (!res.data.data) {
            ...
            this.relatedPictureList = this.relatedPictureList.concat(
              res.data.data.filter((item) => !(item.xrestrict === 1 || item.sanityLevel > (this.user && this.user.id ? 3 : 2)))
            );
          }
        })
        .catch(err => {
